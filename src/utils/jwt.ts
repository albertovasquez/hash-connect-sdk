...
export const parseJwt = (token: string): { exp: number; [key: string]: any } | null => {
    try {
        if (!token || typeof token !== 'string') {
            logError('Invalid token provided to parseJwt');
            return null;
        }

        const parts = token.split(".");
        if (parts.length !== 3) {
            logError('Invalid JWT format: expected 3 parts');
            return null;
        }
        ...
        if (!parsed.exp || typeof parsed.exp !== 'number') {
            logError('JWT missing or invalid exp claim');
            return null;
        }
...
export const isExpired = (token: string): boolean => {
    try {
        if (!token) {
            logWarn('No token provided to isExpired');
            return true;
        }

        const parsed = parseJwt(token);
        if (!parsed || !parsed.exp) {
            logWarn('Unable to parse token or missing expiration');
            return true;
        }
